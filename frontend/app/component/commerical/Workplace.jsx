...
const Workplace = () => {
  const [isBookSiteOpen, setIsBookSiteOpen] = useState(false);

  return (
    <>
      <section className="w-full pb-[35px] lg:py-[70px]">
        <div className="mx-auto max-w-[1525px] rounded-[34px] px-5 sm:px-8 lg:px-[70px]">
          <div className="relative overflow-hidden  rounded-[10px] md:rounded-[34px]">
            <Image
              src="/vega-street/workplace.jpg"
              alt="Workplace"
              width={1400}
              height={260}
            ...
            <div className="absolute inset-0 flex translate-y-[30px] flex-col items-start justify-end gap-4 px-6 py-6 md:flex-row md:items-center md:justify-between md:gap-6 md:px-10">
              ...
              <button
                type="button"
                onClick={() => setIsBookSiteOpen(true)}
                className="group relative isolate mt-2 hidden -translate-y-[30px] shrink-0 cursor-pointer items-center gap-1 overflow-hidden rounded-full bg-white px-2 py-3 font-montserrat text-[12px] font-normal leading-[100%] transition-colors duration-300 hover:bg-[#652A27] md:mt-28 md:inline-flex md:px-3 md:py-3 lg:px-6 lg:py-4 md:text-[15px] lg:text-[20px]"
              >
                <span
                  aria-hidden
      ...
      <BookASite open={isBookSiteOpen} onClose={() => setIsBookSiteOpen(false)} />
    </>
  );
};
